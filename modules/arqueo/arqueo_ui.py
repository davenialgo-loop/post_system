...
def _btn(parent, text, cmd, bg, icon="", **kw):
    ...
    lbl = tk.Label(parent, text=t, font=(FONT,10,"bold"),
                   bg=_DISABLED, fg="#ffffff", cursor="arrow", padx=14, pady=8, **kw)
    lbl.bind("<Enter>",           lambda _: lbl.config(bg=_dk(bg)) if _state["on"] else None)
    lbl.bind("<Leave>",           lambda _: lbl.config(bg=bg if _state["on"] else _DISABLED))
    lbl.bind("<ButtonRelease-1>", lambda _: (lbl.config(bg=_dk(_dk(bg))), lbl.after(120, lambda: lbl.config(bg=bg)), cmd()) if _state["on"] else None)
    def enable():  _state["on"]=True;  lbl.config(bg=bg,fg="#ffffff",cursor="hand2")
    def disable(): _state["on"]=False; lbl.config(bg=_DISABLED,fg="#ffffff",cursor="arrow")
    lbl.enable = enable; lbl.disable = disable
    return lbl
...
class ArqueoModule:
    ...
    def _build(self):
        ...
        # Card estado
        self._status_card_outer = tk.Frame(left, bg=THEME["card_border"])
        self._status_card_outer.pack(fill='x', pady=(0,14))
        self._status_card = tk.Frame(self._status_card_outer, bg=THEME["card_bg"], padx=20, pady=20)
        self._status_card.pack(fill='x', padx=1, pady=1)

        # Card acciones
        act_outer = tk.Frame(left, bg=THEME["card_border"])
        act_outer.pack(fill='x', pady=(0,14))
        act_inner = tk.Frame(act_outer, bg=THEME["card_bg"], padx=20, pady=16)
        act_inner.pack(fill='x', padx=1, pady=1)
        ...
        hist_outer = tk.Frame(right, bg=THEME["card_border"])
        hist_outer.pack(fill='both', expand=True)
        hist_inner = tk.Frame(hist_outer, bg=THEME["card_bg"])
